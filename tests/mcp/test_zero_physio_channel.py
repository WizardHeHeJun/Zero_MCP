...
class TestChannelToWireMuVZero:
    ...
    该路径上没有任何产品码会去**归零** μv，故通道侧一旦写出非零 μv 就会一路到达出网口。

    ⚠ **失败形态已随 M9 落地而变（2026-07-29）**：原文案「守卫不会报错、非零 μv 直接上 wire」
    在 M9（出线 physio 流 μv≠0 即 raise）之后不再成立——同样的通道退化今天会让下面的
    `build_external_priors_override` **直接抛 ValueError**，本用例照样红，只是红在那一行而非
    `assert mu[0] == 0.0`。两种红都指向同一个根因；本类的价值是把「真通道 → wire」这条端到端
    路径钉住，M9 只保证「越过出网口的载荷合规」，保证不了通道本身没退化。
    （真实上界收紧值 = `2·T/√2 − MIN`；对外文书写作 ≈0.2536 是向上取整口径，判据一律现算。）
    ...
    async def test_channel_priors_reach_wire_with_zero_mu_v(
        ...
        for name, mu, _precision in physio_streams:
            assert mu[0] == 0.0, (
                ...
                "且违反跨仓协议 §6-8「physio 对效价盲」；须先跨仓重开该条，再把上界收紧到 "
                "2·T/√2 − MIN（≈0.2536 是向上取整的对外口径，判据现算不手抄）"
            )
