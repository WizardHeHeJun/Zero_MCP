...
- **`close()` 与在途握手（code-review 复核轮 WARN，已收口）**：关停信号精确落在某后端 spawn+握手窗口内时，`close()` 现在会先关 `pendingTransport`（立即终止已 spawn 的子进程、让握手尽快失败——也避免关停被握手超时拖住最长 15s），再 `await connecting` 结算（迟到成功分支自清理、失败分支重启调度因 closed 为 no-op），保证返回时无在途句柄；回归用例见 `tests/unit/backendManager.test.ts`「close() 发生在 connectWithTimeout 在途期间」。
