...
⚠ **M8 语义口径（2026-08-31 随 Zero「方向①解耦」落地同步，判定逻辑与 0.359 均未动）**：
Zero 侧已把「合格判据」（σ≤1.0 + 三条件）与「可声明 Πa」解耦——前者自此只承担
准入/排除语义（σ≤1.0 仅表示「未被最坏情形论证排除」，**不等于**可按 Πa=1/σ² 声明
进融合）；后者受点燃语义界约束（2·SALIENCE_THRESHOLD−MIN_PRECISION=0.359，随
Zero 常量联动非固定数）。因此 **M8 拦的不是「不合格的流」，拦的是「声明超出点燃
语义允许的 Πa」——与合格与否是两个判定**。配套事实（Zero 数学席 2026-08-31 复算）：
`[0.359, 1.0)` 是「不合格但能点燃」的**负边际双输区**（例：hrv σ=1.6 不合格，但其
诚实 Πa≈0.391 已 > 0.359）⇒ M8 拦截面**同时含**合格流与「不合格流的诚实声明」，
任何「被 M8 拦的必是合格流」的推导假设不成立；且 M8 是当前唯一挡住该双输区的
结构约束（Zero 侧对 Πa 无 M2 级结构压制，其 γ 提案要补的正是这一半）。
来源：`notes/2026-08-31-to-mcp-physio-criterion-wording-and-debias.md` §一/§三。
"""
